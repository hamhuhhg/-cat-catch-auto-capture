    ...
    class CatCatcher {
        ...
        createUI() {
            ...
            <div id="catCatch" style="${style}">
                ...
                {/* <label><input type="checkbox" id="ffmpeg" ${localStorage.getItem("CatCatchCatch_ffmpeg") || ""} ${checkboxStyle}><span data-i18n="ffmpeg">استخدام ffmpeg للدمج</span></label> */} {/* معطل لأنه تم استبداله بأزرار الراديو */}
            ...
            // 初始化显示
            this.tips.innerHTML = this.i18n("waiting", "انتظار تشغيل الفيديو"); // تم استخدام i18n هنا بالفعل
        ...
        resetVideoPlayback(video) {
            ...
            const timer = setInterval(() => {
                if (!video.paused) {
                    ...
                    const checkHead = this.catCatch.querySelector("#checkHead");
                    if (checkHead) checkHead.checked = true;
                    this.clearCache(); // قد تحتاج إلى clearCache(true) هنا إذا كان هذا يعتبر بداية جديدة
                    clearInterval(timer);
                }
            }, 500);
            ...
            video.addEventListener('play', () => {
                if (!video.isResetCatCatch) {
                    ...
                    const checkHead = this.catCatch.querySelector("#checkHead");
                    if (checkHead) checkHead.checked = true;
                    this.clearCache(); // قد تحتاج إلى clearCache(true) هنا
                }
            }, { once: true });
        }
        ...
        catchDownload() {
            ...
            if (performMerge) {
                ...
                for (let key in activeMedia) {
                    ...
                    if (lastHeaderIndex === -1 && (currentStream.mimeType?.startsWith("video/") || currentStream.mimeType?.startsWith("audio/"))) {
                        alert(this.i18n("noHeadForMerge", "لم يتم الكشف عن بيانات رأس صالحة للدمج لمسار واحد على الأقل. جرب التنزيل بشكل منفصل.")); // ترجمة
                        hasValidStreamsForMerging = false;
                    }
                    if (lastHeaderIndex > 0) {
                        if (!userConfirmedHeadChoice && !checkHead.checked) {
                            checkHead.checked = window.confirm(this.i18n("headDataConfirmation", "تم الكشف عن بيانات رأس إضافية، هل ترغب في تنظيفها؟")); // ترجمة
            ...
            if (this.tips) {
                this.tips.innerHTML = this.i18n("downloadOrMergeCompleted", "اكتمل التنزيل/الدمج"); // ترجمة
            }
        }
        ...
        downloadDirect(mediaToDownload) {
            ...
            a.remove();
            if (downloadCount === 0) {
                alert(this.i18n("noValidDataForDirectDownload", "لم يتم العثور على بيانات صالحة للتنزيل المباشر.")); // ترجمة
            }
        }
        ...
        clearFileName(obj = "selector", warning = "") {
            ...
            if (element) element.innerHTML = this.i18n("notSet", "غير معين"); // ترجمة
            this.getFileName();
