...
chrome.runtime.onMessage.addListener(function (Message, sender, sendResponse) {
    ...
    if (Message.Message === "getCaptureSettings") {
        if (sender.tab && sender.tab.id) {
            const settingsForClient = {
                ...
                captureDownloadMode: G.captureDownloadMode || "ffmpeg" // Use the new setting
            };
    ...
    // Handle setCaptureDownloadMode from catch-script/catch.js (via content-script.js)
    if (Message.Message === "setCaptureDownloadMode") {
        if (Message.mode && ["ffmpeg", "mp4box", "direct"].includes(Message.mode)) {
            ...
            chrome.storage.sync.set({ captureDownloadMode: G.captureDownloadMode }, () => {
                if (chrome.runtime.lastError) {
                    ...
                    // console.log("CatCatch: captureDownloadMode saved to sync:", G.captureDownloadMode);
